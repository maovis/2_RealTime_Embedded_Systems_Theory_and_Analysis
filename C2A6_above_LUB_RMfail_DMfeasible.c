...
// Service_3, S3, T3= 7,  C3=1, D=T
// Service_4, S4, T4=13,  C4=2, D=T
//
...
// Service_3 - 14.2857 Hz, every 7th Sequencer loop
// Service_4 - 7.6923 Hz,  every 13th Sequencer loop
//
...
// Service_3 = RT_MAX-3	@ 14.2857 Hz   T= 7 
// Service_4 = RT_MAX-4	@ 7.6923 Hz    T=13 
//
...
#define FIBONACCI_10MS_LOAD  750000
#define FIBONACCI_20MS_LOAD 1500000
...
static unsigned int __attribute__((noinline)) calcFibonacci (unsigned int count)
{
    unsigned int f0=0, f1=1, i=0, count_ = count, temp_=0;
    for(i=0;i<count_;i++)
    {
        temp_ = f0+f1;
        f0 = f1;
...
int main(void)
{
    ...
    //Fibo warm up test
    calcFibonacci(FIBONACCI_10MS_LOAD);
    calcFibonacci(FIBONACCI_20MS_LOAD);
...
void *Service_1(void *threadp)
{
    ...
    while(!abortS1)
    {
        ...
        syslog(LOG_CRIT, "Thread 1 start %llu @ %lf on core 3\n", S1Cnt, current_time);
        calcFibonacci(FIBONACCI_10MS_LOAD);
        //current_time=getTimeMsec();
        //syslog(LOG_CRIT, "Thread 1 end   %llu @ %lf on core 3\n", S1Cnt, current_time);
    }
...
void *Service_2(void *threadp)
{
    ...
    while(!abortS2)
    {
        ...
        syslog(LOG_CRIT, "Thread 2 start %llu @ %lf on core 3\n", S2Cnt, current_time);
        calcFibonacci(FIBONACCI_10MS_LOAD);
        //current_time=getTimeMsec();
        //syslog(LOG_CRIT, "Thread 2 end   %llu @ %lf on core 3\n", S2Cnt, current_time);
    }
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        syslog(LOG_CRIT, "Thread 3 start %llu @ %lf on core 3\n", S3Cnt, current_time);
        calcFibonacci(FIBONACCI_10MS_LOAD);
        //current_time=getTimeMsec();
        //syslog(LOG_CRIT, "Thread 3 end   %llu @ %lf on core 3\n", S3Cnt, current_time);
    }
...
void *Service_4(void *threadp)
{
    ...
    while(!abortS4)
    {
        ...
        syslog(LOG_CRIT, "Thread 4 start %llu @ %lf on core 3\n", S4Cnt, current_time);
        calcFibonacci(FIBONACCI_20MS_LOAD);
        //current_time=getTimeMsec();
        //syslog(LOG_CRIT, "Thread 4 end   %llu @ %lf on core 3\n", S3Cnt, current_time);
    }
